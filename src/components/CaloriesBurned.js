...
const CaloriesBurned = () => {
  ...
  const syncData = () => {
    ...
    setCaloriesData([
      ...
      { day: 'Wednesday', calories: 400 },
      { day: 'Thursday', calories: 350 },
      { day: 'Friday', calories: 500 },
      { day: 'Saturday', calories: 600 },
      { day: 'Sunday', calories: 450 },
    ]);
  };
  ...
  const data = {
    labels: caloriesData.map(point => point.day), // Using the day as labels
    datasets: [
      {
        label: 'Calories Burned',
        data: caloriesData.map(point => point.calories),
        fill: false,
  ...
  const options = {
    ...
    plugins: {
      ...
      title: {
        display: true,
        text: 'Weekly Calories Burned Data',
        color: 'rgba(0, 0, 0, 0.7)', // Set this for light mode
      },
    },
    scales: {
  ...
  return (
    <div className="bg-white/10 dark:bg-white/5 p-4 rounded-lg shadow-lg w-full dark:text-white backdrop-filter backdrop-blur-lg border border-white/20 dark:border-gray-600">
      <h2 className="text-2xl font-bold mb-4">Calories Burned Data</h2>
